...
class IGraph {
   public:
      ...
      virtual const IGraphNode *
      NearestNode(std::vector<float> point,
                  const DistanceFunction &distance) const = 0;
      virtual const std::vector<std::vector<float>>
      GetPath(std::vector<float> src, std::vector<float> dest,
...
class GraphBase : public IGraph {
   public:
      ...
      const IGraphNode *NearestNode(std::vector<float> point,
                                    const DistanceFunction &distance) const;
      const std::vector<std::vector<float>>
      GetPath(std::vector<float> src, std::vector<float> dest,
              const RoutingStrategy &strategy) const;
};
