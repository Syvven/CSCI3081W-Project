...
class AStar : public RoutingStrategy {
   public:
      AStar()
          : cost(new EuclideanDistance()), heuristic(new EuclideanDistance()) {}
      AStar(DistanceFunction *cost, DistanceFunction *heuristic)
          : cost(cost), heuristic(heuristic) {}
      virtual ~AStar();

      std::vector<std::string> GetPath(const IGraph *graph,
