...
class OsmParser {
   ...
   private:
      ...
      static void read_adjacencies_to(OSMGraph *graph, pugi::xml_document *doc,
                                      bool debug = false);
      static unordered_map<string, set<string>>
      get_adjacency_list_from_file(pugi::xml_document *doc, bool debug = false);
      ...
      static float getLon(float lat, float lon, float centerLat,
                          float centerLon);
};
