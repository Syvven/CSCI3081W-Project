...
unordered_map<V, int> *
count_value_occurrences(const unordered_map<T, V> *to_count) {
...
template <class T> T argmax(const unordered_map<T, int> *mapping) {
...
unordered_map<T, bool> *where_equal(const unordered_map<T, int> *mapping,
   ...
   for (auto kv : *mapping) {
      result->insert({kv.first, kv.second == equal_to});
   }
...
GraphUtils::ConnectedComponents(const IGraph *graph) {
...
OSMGraph *GraphUtils::Filter(const IGraph *original,
                             const unordered_map<string, bool> *filter) {
   // NOTE: does not preserve the type of the graph that was input, always gives
   // back an OSMGraph
      ...
      if (include) {
         ...
         if (existing_node) {
            ...
            filtered_graph->AddNode(
               new OSMNode(Point3(loc.at(0), loc.at(1), loc.at(2)),
...
OSMGraph *
GraphUtils::FilterToLargestConnectedComponent(const IGraph *original) {
...
void GraphUtils::dfs_visit(const IGraphNode *n,
                           unordered_map<string, int> *record, int index) {
...
OSMGraph *OsmParser::read_nodes(pugi::xml_document *doc, bool debug) {
   ...
   for (way_node = parent_of_nodes.child("node"); way_node != nullptr;
      ...
      graph->AddNode(new OSMNode(Point3(longitude, height, latitude), id));
   }
...
float OsmParser::getLon(float lat, float lon, float centerLat,
                        float centerLon) {
...
void OsmParser::read_adjacencies_to(OSMGraph *graph, pugi::xml_document *doc,
                                    bool debug) {
...
OsmParser::get_adjacency_list_from_file(pugi::xml_document *doc, bool debug) {
   ...
   // create a parent of all (interesting) nodes, then get the first node the
   // the "way" name
   pugi::xml_node parent_of_nodes = doc->first_child();
   ...
   while (way_node != NULL) {
      for (pugi::xml_node tag_node = way_node.child("tag"); tag_node != nullptr;
         ...
         if (tag_node != NULL &&
            ...
            highway_way_nodes.insert(
               {way_node.attribute("id").value(), way_node});
            break;
         }
      }
   ...
   for (auto way_node_key_value : highway_way_nodes) {
      ...
      while (second != NULL) {
         ...
         if (strcmp(first.name(), "nd") == 0 &&
             strcmp(second.name(), "nd") == 0) {
            adjacency_list[first.attribute("ref").value()].insert(
               second.attribute("ref").value());
            adjacency_list[second.attribute("ref").value()].insert(
               first.attribute("ref").value());
            num_edges++;
         }
         first = second;
