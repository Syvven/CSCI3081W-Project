...
/*const vector< vector<float> > OSMGraph::GetPath(vector<float> src,
vector<float> dest) const { const IGraphNode* start_node =
entity_project::NearestNode(this, src); const IGraphNode* end_node =
entity_project::NearestNode(this, dest);

  vector<string>* string_path = entity_project::DjikstraPathing::GetPath(this,
start_node->GetName(), end_node->GetName());

  vector< vector<float> > position_path =
entity_project::ConvertPathToFloatVectors(*this, *string_path);
