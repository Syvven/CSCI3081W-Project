...
template <class T> class FStack {
   public:
      ...
      FStack *Push(T elem) const { return new FStack<T>(elem, this); };
...
struct CandidatePath {
      CandidatePath(FStack<string> *path, float distance = 0,
                    float estimate = 0)
...
vector<string> AStar::GetPath(const IGraph *graph, const std::string &from,
   ...
   // visited.insert(from);
   possible_paths.push(new CandidatePath(new FStack<string>(from), 0));
   ...
   while (!possible_paths.empty()) {
      ...
      if (candidate->path->ToList()->size() < 10) {
          ...
          while(!possible_paths.empty()) {
              std::cerr << "=============" << "REMAINING PATHS" <<
      "==============" << std::endl; CandidatePath* candidate =
      possible_paths.top(); possible_paths.pop();
...
DepthFirstSearch::GetPath(const IGraph *graph, const std::string &from,
                          const std::string &to) const {
   unordered_set<string> visited; // don't check nodes we've already visited
   queue<CandidatePath *>
      possible_paths; // queue of all paths we're considering in BFS
   ...
   while (!possible_paths.empty()) {
      ...
      for (IGraphNode *next : next_steps) {
         ...
         if (visited.find(next_name) == visited.end()) {
            ...
            possible_paths.push(new CandidatePath(path->path->Push(next_name)));
         }
      }
   }
}
