...
const IGraphNode *
GraphBase::NearestNode(std::vector<float> point,
                       const DistanceFunction &distanceFunction) const {
   const std::vector<IGraphNode *> nodes = GetNodes();
...
GraphBase::GetPath(std::vector<float> src, std::vector<float> dest,
                   const RoutingStrategy &pathing) const {
   using namespace std;
   ...
   vector<string> string_path =
      pathing.GetPath(this, start_node->GetName(), end_node->GetName());
