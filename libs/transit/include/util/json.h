...
class JsonValue {
   public:
      ...
      /**
       * @brief Create a JsonValue from an existing picojson::value (creates a
       * copy)
       ...
       * picojson::value. Instead of creating a copy of the picojson::value,
       * this just stores a reference to it. This is only needed for internal
       * use.
...
class JsonObject {
   ...
   public:
      ...
      /**
       * @brief Read/write access to a value in this JSON object from a given
       * key. You can add to the JsonObject this way.
       *
       * @param key: the key
      ...
      /**
       * @brief Read only access to a value in this JSON object from a given
       * key. Requires existence of provided key. The returned value IS A COPY.
...
class JsonArray {
   ...
   public:
       ...
       * @throws std::runtime_error: if the index is not within the bounds of
       * the array
       * @param idx: the index
...
class JsonSession : public JSONSession {
   public:
       ...
       * server
       */
      virtual void ReceiveCommand(const std::string &cmd, JsonObject &data,
