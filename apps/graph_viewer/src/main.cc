...
void drawPath(Image &image, const routing::BoundingBox &bb,
              std::vector<std::vector<float>> &path, Color color) {
...
int main(int argc, char **argv) {
   ...
   if (argc < 3) {
      std::cout
         << "Usage: ./build/bin/graph_viewer /path/to/graph /path/to/output.png"
         << std::endl;
      return 0;
   }
   ...
   std::vector<float> start =
      graph->NearestNode(bb.min, EuclideanDistance())->GetPosition();
   std::vector<float> end =
      graph->NearestNode(bb.max, EuclideanDistance())->GetPosition();
