...
function changeView() {
   ...
   if (currentView >= 0) {
      displayJSON(entities[currentView]);
      controls.target.copy(entities[currentView].position);
      controls.update();
   }
}
...
function loadScene(json) {
   ...
   for (let i = 0; i < json.length; i++) {
      console.log(json[i]);
      if (json[i].command == "SetScene") {
         console.log(json[i]);
         sceneModel = json[i].params.mesh;
         sceneTexture = json[i].params.texture;
         sceneScale = json[i].params.scale;
         scenePosition = json[i].params.position;
      } else if (json[i].command == "AddMesh") {
         addMesh(json[i].params);
      } else {
         api.sendCommand(json[i].command, json[i].params);
      }
   }
   loadModels();
}
...
function addEntity(data) {
   $("#entitySelect").append($(
      ...
      + data.details.id + ": " + data.details.name
      + '</option>'
   ));
