   ...
   public:
      ...
      void ReceiveCommand(const std::string &cmd, JsonObject &data,
         ...
         if (cmd == "CreateEntity") {
            ...
            for (std::map<int, const IEntity *>::iterator it =
                    updateEntites.begin();
      ...
      void UpdateEntity(const IEntity &entity) {
         ...
         SendEntity("UpdateEntity", entity, true);
      }
...
class TransitWebServer : public WebServerBase, public IController {
   public:
      ...
      void service(int time = 10) {
         WebServerBase::service(time);
         std::vector<IEntity *> entities = model.GetEntities();
         for (int i = n_clients; i < sessions.size(); i++) {
            for (int j = 0; j < entities.size(); j++)
               static_cast<TransitService *>(sessions[i])
                  ->AddEntity(*entities[j]);
         }
         n_clients = sessions.size();
      }
